...
bool testASAddAndSize()
{
   AmountSet AS = asCreate();
   for (int i = 1; i < 1000; ++i) {
	   char j = (char) i;
	   ++j;
	   ASSERT_TEST(asRegister(AS, &j) == AS_SUCCESS);
	   ASSERT_TEST(asGetSize(AS) == i);
   }
   asDestroy(AS);
   return true;
}
...
bool testMapGet()
{
   ...
   for (int i = 1; i < 1000; ++i) {
	   ...
	   ++j;
	   ASSERT_TEST(asRegister(AS, &j) == AS_SUCCESS);
	   ASSERT_TEST(asChangeAmount(AS, &j, i) == AS_SUCCESS);
	   ASSERT_TEST(asGetSize(AS) == i);
   }
   double* getVal;
   for (int i = 1; i < 1000; ++i) {
	   ...
	   ASSERT_TEST(asGetAmount(AS, &j, getVal) == AS_SUCCESS);
	   ASSERT_TEST(*getVal == i);
   }
   int i = 0;
   char j = (char) i;
   ASSERT_TEST(asGetAmount(AS, &j, getVal) == AS_ITEM_DOES_NOT_EXIST);
   i = 1000;
   j = (char) i;
   ASSERT_TEST(asGetAmount(AS, &j, getVal) == AS_ITEM_DOES_NOT_EXIST);
   asDestroy(AS);
